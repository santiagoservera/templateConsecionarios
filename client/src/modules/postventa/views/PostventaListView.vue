  ...
  <div class="p-6 space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">{{ meta.total }} caso{{ meta.total !== 1 ? 's' : '' }} en total</p>
      </div>
    ...
    <template v-if="tab === 'services'">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-0.5">Seguimiento de mantenimiento periódico y garantía por vehículo.</p>
        </div>
      ...
      <div v-else-if="!services.length" class="text-center py-20 bg-white dark:bg-[#1a1a2e] rounded-2xl border border-slate-200 dark:border-white/10">
        ...
        <p class="font-medium text-slate-500">Sin services registrados</p>
            ...
            <div class="shrink-0 text-right flex flex-col items-end gap-2">
              <span class="text-[10px] font-semibold px-2.5 py-1 rounded-full"
                :class="{
                  ...
                  'bg-slate-100 text-slate-500': srv.estado === 'CANCELADO',
                }">
